...
def reconocer_cadena(afd, cadena, rm=False):
    ...
    indice_inicio = 0  # Índice de inicio de la cadena actualmente analizada
    coincidencias = []  # Lista para almacenar las cadenas reconocidas

    while indice_inicio < len(cadena):
        estado_actual = afd  # Reiniciar al estado inicial para cada nueva subcadena
        indice_aceptado = None  # Índice del último estado de aceptación encontrado

        for indice, simbolo in enumerate(cadena[indice_inicio:], start=indice_inicio):
            simbolo_normalizado = simbolo.replace('\n', '\\n').replace('\t', '\\t')
            transicion = estado_actual.transiciones.get(simbolo_normalizado, [])
            
            if transicion:
                estado_actual = transicion[0]  # Avanzar al siguiente estado
                if estado_actual.isAceptacion:
                    indice_aceptado = indice  # Actualizar el índice del último estado de aceptación
            else:
                break  # No hay transición válida, salir del bucle interno

        # Si se encontró un estado de aceptación, agregar la subcadena a las coincidencias
        if indice_aceptado is not None:
            coincidencia = cadena[indice_inicio:indice_aceptado + 1]
            coincidencias.append(coincidencia)
            indice_inicio = indice_aceptado + 1  # Continuar desde el siguiente carácter después del último estado de aceptación
        else:
            indice_inicio += 1  # Avanzar al siguiente carácter si no se encontró ninguna coincidencia

    # Opcionalmente, eliminar las coincidencias encontradas de la cadena original
    if rm:
        for encontrada in coincidencias:
            cadena = cadena.replace(encontrada, '')
        return coincidencias, cadena
